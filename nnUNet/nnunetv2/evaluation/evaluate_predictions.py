...
import nnunetv2.utilities.surface_distance as surface_distance ##240814
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        ##24 By Wenzhao
        pred_mat = np.squeeze(mask_pred)#
        gt_mat = np.squeeze(mask_ref)#
        #print(gt_mat.shape)

        sx, sy, sz  = seg_ref_dict['spacing'] #
        #print(seg_ref_dict['spacing'])
        ...
        if np.all(gt_mat == 0) and np.all(pred_mat == 0):
            ...
            full_sdc = surface_distance.compute_surface_dice_at_tolerance(full_sd, 1.0)
        
        results['metrics'][r]['HD95'] = full_hd95 ##24
        results['metrics'][r]['SDC'] = full_sdc ##24
        #print('full_hd95')
        #print(full_hd95)
        #print('full_sdc')
        #print(full_sdc)
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    for m in metric_list:    
        ...
        for ii in np.arange(len(results)):
            #for r in regions_or_labels:
            #means_case[m][r] = np.nanmean([i['metrics'][r][m] for i in results])
            #breakpoint()
            means_case[m][ii] = np.nanmean([results[ii]['metrics'][r][m] for r in regions_or_labels])
    
    ## By Wenzhao Zhao
    print("log_metric24")
    #breakpoint()
    log_file = join(folder_pred,'log_metric24.txt')
    with open(log_file, 'a+') as f:
        f.write("dice\n")
        for a in np.arange(len(results)):

            f.write(str(means_case['Dice'][a]))

            f.write("\n")
        f.write("hd95\n")
        for a in np.arange(len(results)):

            f.write(str(means_case['HD95'][a]))

            f.write("\n")
        f.write("surface dice\n")
        for a in np.arange(len(results)):

            f.write(str(means_case['SDC'][a]))
