...
from nnunetv2.utilities.get_network_from_plans_MedNextV1 import get_network_from_plans
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), equivariance_test: bool = False):
        ...
        self.equivariance_test = equivariance_test
        ...
        #self.initial_lr = 1e-2 ##original nnUNet ############################################
        #self.weight_decay = 3e-5 ##original #
        #adamw
        self.initial_lr = 1e-3 #/10 
        self.weight_decay = 0.01  #23 #3e-5 #3e-5 ##original #
        #adam
        ...
        self.num_epochs = 120#1000 
        self.current_epoch = 0
        
        if equivariance_test:
            self.initial_lr = 1e-3 /10 
        #############################################
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        #return get_network_from_plans_fb(plans_manager, dataset_json, configuration_manager,
        #                              num_input_channels, deep_supervision=enable_deep_supervision)
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.network.parameters(), lr = self.initial_lr, eps=1e-08, weight_decay=self.weight_decay,)
        #optimizer = torch.optim.Adam(self.network.parameters(), lr=self.initial_lr, weight_decay=self.weight_decay)
        ##optimizer = torch.optim.AdamW(self.network.parameters(), lr = self.initial_lr, eps=1e-04, weight_decay=self.weight_decay,)
        #https://github.com/MIC-DKFZ/MedNeXt/blob/main/nnunet_mednext/training/network_training/MedNeXt/nnUNetTrainerV2_MedNeXt.py
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            dl_tr = nnUNetDataLoader3D(dataset_tr, self.batch_size,
                                       ...
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None,
                                       full_aug = self.equivariance_test)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if self.local_rank == 0:
            ...
            self.print_to_log_file("Mean Validation Surface Dice: ", (metrics['foreground_mean']["SDC"]), also_print_to_console=True)#24
            self.print_to_log_file("Mean Validation HD95: ", (metrics['foreground_mean']["HD95"]), also_print_to_console=True)#24
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            if self.on_train_epoch_end(train_outputs)==False: ##24
                break
